...
public class FbLeadExtractTest {

	private static String fbFileLeadName = "FbLeadsNewAfter28ANDBefJan16";
	...
	public static String fbSelectQuery = "select CUST_NAME ,EMAIL ,NOTES ,PHONE,SRC from T_LEAD " + "where "
			...
			+ "and EMAIL  =  '%3$s' and CONVERT(VARCHAR(MAX), NOTES) = '%4$s' and PHONE = '%5$s' and  src='%6a$s' ";
		...
		try {
			...
			for (String leadId : FbExtractConstants.fbLeadIds) {
				if (fbLeadMap.containsKey(leadId)) {
					...
					String selectQuery = String.format(fbSelectQuery, "2023-01-17", fbLeadReq.getCustomerFullName(),
	...
	public static void processResponse(HashMap<String, FBLeadRequest> fbLeadMap, Set<String> set) throws IOException, ParseException {
		Set<String> setMor = new HashSet();
		Set<String> leadId = new HashSet();
		File myObj = getFile(fbFileLeadName);
		FileWriter fileWriter = new FileWriter(myObj);
		...
		try {
			...
			for (Entry<String, FBLeadRequest> entry : fbLeadMap.entrySet()) {
				...
				if(createdDate.after(afterDate) && createdDate.before(beforeDate)) {
					if(!leadId.contains(entry.getKey())) {
						String json = mapper.writeValueAsString(entry.getValue());
						if (!set.contains(json) && !setMor.contains(json)) {
							System.out.println("Count :: " + count++);
							setMor.add(json);
							...
							fileWriter.write(mapper.writeValueAsString(entry.getValue()) + "  \n");
						} else {
							System.out.println("json ::"+json);
						}
						leadId.add(entry.getKey());
					}
				}
			}
		} finally {
